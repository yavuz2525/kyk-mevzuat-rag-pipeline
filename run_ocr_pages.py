"""
DeepSeek OCR - Process first 3 pages of PDF with per-page output directories.
"""
import os, sys, re, io
import numpy as np
...
def draw_boxes_and_extract(image, refs, page_dir, page_idx):
    ...
    boxed_path = os.path.join(page_dir, f"page_{page_idx + 1}_boxed.png")
...
def md_to_html(md_text):
    ...
    parts = [
        ...
        ".boxed-ref{display:inline-block;background:#fff3cd;border:1px solid #ffc107;border-radius:4px;padding:1px 6px;margin:0 2px;font-size:0.8em;color:#856404}",
        "</style></head><body>",
    ]
    in_table = False
...
def clean_output(content, matches_img, matches_other):
    ...
    for idx, m in enumerate(matches_img):
        content = content.replace(m, f"![](images/{idx}.jpg)\n")
...
def main():
    ...
    for i in range(NUM_PAGES):
        img = pdf_page_to_image(PDF_PATH, i, DPI)
        images.append(img)
    ...
    # --- Load model ---
    print("[2/3] Loading DeepSeek OCR model...")
    ...
    # --- Process each page ---
    print("[3/3] Running OCR on 3 pages...")
    for page_idx in range(NUM_PAGES):
        page_dir = os.path.join(OUTPUT_BASE, f"page_{page_idx + 1}")
        os.makedirs(page_dir, exist_ok=True)
        img_path = os.path.join(TEMP_PAGES_DIR, f"page_{page_idx + 1}.png")
        ...
        if output_text is None:
            print(f"    No output for page {page_idx + 1}")
            continue
        ...
        boxed_path = draw_boxes_and_extract(
            images[page_idx].copy(), matches_ref, page_dir, page_idx
        )
        ...
        images[page_idx].save(orig_path)
    ...
    for i in range(NUM_PAGES):
        d = os.path.join(OUTPUT_BASE, f"page_{i+1}")
        print(f"  {d}/")
